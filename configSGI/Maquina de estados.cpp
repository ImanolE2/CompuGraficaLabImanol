...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (dogAnim == 1) // caminar
	{
		if (!step)
		{
			//1.0f para que las patas, cabeza y cola se muevan 10 veces mas rapido
			RLegs += 1.0f;
			FLegs += 1.0f;
			head += 1.0f;
			tail += 1.0f;
		...
		// 0.001f para que avance a una velocidad decente
		dogPos.z += 0.01f;
	}

	if (dogAnim == 2) // animo tail para parar las demas partes
	{
		if (!step)
		{
			// Aumentamos la velocidad a la que mueve la cola cuando esta quieto
			tail += 1.0f;
			if (tail >= 15.0f)
				step = true;
		}
